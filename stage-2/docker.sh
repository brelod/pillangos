...
build() {
    docker run -t -it --rm \
        ...
        "$CONF_DOCKER_IMAGE_TAG_STAGE_1" \
        "$CONF_GIT_DIR/stage-2/build.sh"

    docker import \
        --change 'ENTRYPOINT ["/bin/bash"]' \
        "$GIT_DIR/cache/stage-2.tar.gz" \
        "$CONF_DOCKER_IMAGE_TAG_STAGE_2"
}

push() {
    docker push "$CONF_DOCKER_IMAGE_TAG_STAGE_2"
}

pull() {
    docker pull "$CONF_DOCKER_IMAGE_TAG_STAGE_2"
}
